...
本脚本随项目走:安装时复制进项目的 .agent_workspace/,与系统文件(BLACKBOARD.md /
board.html / roles/)和状态数据(state.json / tasks/ / inbox/ / archive.json)同处一地。
自包含设计——协作者 git clone 后即可直接使用,不依赖任何本机绝对路径。
黑板定位:优先 --ws 参数或 MULTI_AI_WS 环境变量,否则从当前工作目录向上查找
.agent_workspace/。在目标项目根目录下执行即可,新项目先用 init 初始化。
...
WS_NAME = ".agent_workspace"

# 运行时由 set_ws() 绑定;除 init 外的子命令入口统一先 resolve_ws()。
WS = None
STATE_FILE = None
ARCHIVE_FILE = None
TASKS_DIR = None
...
def set_ws(ws_dir):
    global WS, STATE_FILE, ARCHIVE_FILE, TASKS_DIR, INBOX_DIR
    WS = ws_dir
...
def resolve_ws(cli_ws=None):
    """定位黑板目录:--ws/MULTI_AI_WS > 从 cwd 向上查找 > 旧布局(脚本在黑板内)。"""
    raw = cli_ws or os.environ.get("MULTI_AI_WS")
    if raw:
        cand = Path(raw).expanduser().resolve()
        if cand.name != WS_NAME:
            cand = cand / WS_NAME
        if not (cand / "state.json").exists():
            die(f"--ws/MULTI_AI_WS 指定位置没有黑板(缺 state.json): {cand}")
        return cand
    ...
    if here.name == WS_NAME and (here / "state.json").exists():
        return here
    die("未找到黑板:请在目标项目根目录(或其子目录)下执行,或用 --ws 指定项目根;新项目先运行 init")
...
def cmd_init(args):
    ...
    if (ws / "state.json").exists():
        die(f"黑板已存在: {ws}(无需重装;旧布局迁移只需删除项目内系统文件副本,数据不动)")
    project = args.project or root.name
    (ws / "tasks").mkdir(parents=True, exist_ok=True)
    for agent in AGENTS:
        (ws / "inbox" / agent).mkdir(parents=True, exist_ok=True)
    set_ws(ws)
    save_state({
        "project": project,
        "lock": {
            "holder": None,
            "since": None,
            "note": "防呆提示用。holder 非空表示上一个 Agent 可能未收尾,启动的 Agent 应提醒人类确认,而非拒绝工作。",
        },
        "agents": {
            "claude": {"role": "chief_architect", "last_active": None},
            "codex": {"role": "deputy_architect_and_lead_developer", "last_active": None},
            "trae": {"role": "feature_developer", "last_active": None},
        },
        "tasks": [],
    })
    ok(f"黑板已初始化: {ws}(project: {project})")
    print("[bb] 下一步:把 bb.py/BLACKBOARD.md/board.html/roles 从 skill 母本复制进本目录,并生成 .multi-ai-version 指纹")
...
def main():
    p = argparse.ArgumentParser(
        ...
        epilog=(
            "黑板定位: 在目标项目根目录下执行,或 --ws <项目根>(env: MULTI_AI_WS)\n"
            "典型流程:\n"
            "  init --project demo           新项目初始化(只建状态数据骨架)\n"
    ...
    p.add_argument("--ws", help="项目根目录(或其 .agent_workspace);缺省从当前目录向上查找")
    sub = p.add_subparsers(dest="cmd", required=True)

    s = sub.add_parser("init", help="初始化黑板:在项目内创建纯状态数据骨架(state.json/tasks/inbox)")
    s.add_argument("--project", help="项目名,缺省取根目录名")
    s.add_argument("--root", help="项目根目录,缺省当前目录")
