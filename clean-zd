...
# clean conda cache（pnpm 缓存由下方带 dry-run 守卫的 pnpm 块统一处理）
if type "conda" &>/dev/null; then
  if [ -z "$dry_run" ]; then
    msg 'Cleaning up conda cache...'
    conda clean --all --yes &>/dev/null      # 未安装的包缓存
    conda clean --tarballs --yes &>/dev/null # tarballs 缓存
    conda clean --index-cache --yes &>/dev/null
  fi
fi

# 清理 ~/Library/Containers

#com.tencent.meeting/Data/Library/Global/Logs
if [ -d ~/Library/Containers/com.tencent.meeting/Data/Library/Global/Logs ]; then
  collect_paths ~/Library/Containers/com.tencent.meeting/Data/Library/Global/Logs/*
  msg 'clean log in com.tencent.meeting'
  remove_paths
fi

# 清理 ~/Library/Developer/
if [ -d ~/Library/Developer/Xcode ]; then
  collect_paths ~/Library/Developer/Xcode/DocumentationCache/*
  collect_paths ~/Library/Developer/Xcode/watchOS\ DeviceSupport/*
  collect_paths ~/Library/Developer/Xcode/UserData/IB\ Support/Simulator\ Devices/*
  collect_paths ~/Library/Developer/CoreSimulator/Devices/*
  msg 'clean Xcode Cache'
  remove_paths
fi

# 清理 Library/Application Support/Caches
if [ -d ~/Library/Application\ Support/Caches ]; then
  collect_paths ~/Library/Application\ Support/Caches/*
  msg 'clean Application Support/Caches'
  remove_paths
fi
